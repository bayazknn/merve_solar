...
def set_seed(seed: int) -> None:
    ...
    Reproducibility is per-device: MPS, CUDA and CPU will not agree bitwise even from the
    same seed, so a published number is reproducible on the device that produced it.
    """
    random.seed(seed)
    np.random.seed(seed)
